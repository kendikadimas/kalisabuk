...
import { ArrowLeft, Save, Users, User, Phone, MapPin, Briefcase, GraduationCap, Calendar, Image as ImageIcon, ExternalLink } from 'lucide-react';
...
export default function Edit({ official }: Props) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        periode_start: official.periode_start || '',
        periode_end: official.periode_end || '',
        order: official.order || 0,
        is_active: official.is_active,
        is_head: official.is_head,
        welcome_message: official.welcome_message || '',
        _method: 'PUT',
    });
        ...
        <AppSidebarLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="p-6 md:p-8 space-y-8 bg-slate-50/50 min-h-full">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">Edit Perangkat Desa</h1>
                        <p className="text-slate-500 text-sm">Perbarui data {official.name}.</p>
                    </div>
                </div>
                                    ...
                                    <div className="border-2 border-dashed border-slate-200 rounded-xl p-8 hover:bg-slate-50 transition-colors text-center cursor-pointer group relative aspect-[3/4] flex flex-col items-center justify-center bg-slate-50/50 overflow-hidden">
                                        ...
                                        {data.photo ? (
                                            ...
                                            <div className="flex flex-col items-center gap-3">
                                                {official.photo && !data.photo ? (
                                                    <div className="absolute inset-0 w-full h-full z-0 p-2">
                                                        <img
                                                            src={`/storage/${official.photo}`}
                                                            alt="Current Photo"
                                                            className="w-full h-full object-cover rounded-md opacity-50 group-hover:opacity-30 transition-opacity"
                                                        />
                                                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                            <ImageIcon className="w-8 h-8 text-emerald-600 mb-2" />
                                                            <p className="text-sm font-semibold text-slate-700">Ganti Foto</p>
                                                            <p className="text-xs text-slate-500">Klik untuk upload baru</p>
                                                        </div>
                                                    </div>
                                                ) : (
                                                    <>
